...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'delete_document') {
    ...
    try {
        ...
        header('Location: ' . '/sibeta/public/index.php?page=upload-teknis');
        exit();
    } catch (Exception $e) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'save_document') {
    try {
        $updatedRows = $mahasiswaController->updateIsSavedByType($nim, 'Teknis');
        if ($updatedRows > 0) {
            header('Location: ' . '/sibeta/public/index.php?page=upload-teknis&status=saved');
            exit();
        }
    } catch (Exception $e) {
        echo '<div class="alert alert-danger">' . $e->getMessage() . '</div>';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'edit_document') {
    ...
    try {
        ...
        if ($file['error'] === UPLOAD_ERR_OK) {
            ...
            $uploadDir = $_SERVER['DOCUMENT_ROOT'] . "/sibeta/app/uploads/teknis/";
            $fileExtension = pathinfo($file['name'], PATHINFO_EXTENSION);
            $uniqueFilename = $nim . '_' . $dokumenID . '_' . time() . '.' . $fileExtension;
            $filePath = "uploads/teknis/" . $uniqueFilename;
            ...
            if (move_uploaded_file($file['tmp_name'], $fullPath)) {
                ...
                if ($updateResult) {
                    ...
                    header('Location: ' . '/sibeta/public/index.php?page=upload-teknis&status=updated');
                    exit();
                } else {
...
$documents = $mahasiswaController->getDocumentsByType($nim, 'Teknis');

$documentCounts = $mahasiswaController->getDocumentCountByType($nim, 'Teknis');

$isComplete = $mahasiswaController->isTeknisDocumentsComplete($nim);

$jenisDokumen = $mahasiswaController->getJenisDokumen('Teknis');
                ...
                if (isset($_GET['status'])):
                    ...
                    switch ($_GET['status']) {
                        ...
                        case 'deleted':
                            ...
                            break;
                        case 'saved':
                            $statusMessage = "Dokumen teknis berhasil disimpan";
                            $statusType = 'success';
                            break;
                        case 'error':
